...
const SystemManagement = (state = initialState, action) => {
    ...
    switch (action.type) {
        ...
        case systemManagementAction.CODE_TABLE_DATA:
            ...
            return newState;
            // 卡点管理
        case 'REQUEST_CARDPOINT_MANAGE_LIST_HUSHI_DATA':
            return {
                ...state,//原状态
                isFetching: true,
                didInvalidate: false
            }
        case 'CardPoint_ManageListHushi-data': //卡点管理
            newState.data.cardPointList.result.list = action.data.result.list;
            newState.data.cardPointList.result.page = action.data.result.page;
            newState.isFetching = false;
            return newState;
